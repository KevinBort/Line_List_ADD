...
# ESTE YA FUNCIONA BIEEEEEEEEEEEEEEEEEEEEN
# COMPLETAR DATOS DE PROYECTO ... Podria hacerse por input de usuario o leerse de algun lado en el archivo base de excel.
codigo_proyecto='4651'
nombre_proyecto='hitachi' # Mucho cuidado aca. no cometer typos porque se rompe todo.
rev='0A'
nombre_planta='Small Scale LBM liquefaction Plant'
...
input_file_name='Line_list_base.xlsx'# Chequear que sea el nombre y extension correctas.
...
try:    
    ...
    for index in range(row_start, parte_2_ws.max_row+1):
        if parte_2_ws.cell(row=index, column=4).value is not None:
            ...
            if Pdis==0:
                continue
            if nps<=25:
                parte_2_ws.cell(row=index, column=38, value='Sound Engineering Practice')
                parte_2_ws.cell(row=index, column=37, value='4.3') 
            elif nps > 25 and Pdis == 'TBD':
                parte_2_ws.cell(row=index, column=38, value='TBD')
                parte_2_ws.cell(row=index, column=37, value='TBD')   
            elif Pdis * nps <= 1000 and nps <= 100:
                parte_2_ws.cell(row=index, column=37, value='1')
                parte_2_ws.cell(row=index, column=38, value='A')
            elif ((nps * Pdis <= 3500) and (25 < nps <= 350)) or (Pdis>35 and 24<=nps<=100): # ESTA ULTIMA HAY QUE AGREGARLA PORUQE NO ESTA INCLUIDA EN LA TABLA.
                parte_2_ws.cell(row=index, column=37, value='2')
                parte_2_ws.cell(row=index, column=38, value='A2,D1,E1')
            elif (nps>350 and Pdis<=10) or (nps*Pdis>3500):
    ...
    print('Paso 1 completo, se generó el archivo line_list_paso_1.xlsx.Completar la columna de presiones de diseño y guardar el archivo como line_list_paso_2.xlsx')
